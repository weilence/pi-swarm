...
- Pi SDK integration is isolated in `src/pi/`: one `Agent` facade over a session host / model settings / stream metrics split, wired as supervisor or sub-agent through the unified `createAgent` factory (`agent-factory.ts`), all on one shared ModelRuntime.
...
接口类型与模型选择。布局上，**聊天区左侧是一个满屏高、带边框的会话边栏**（固定宽度，
垂直排列，右侧为 transcript 与底部输入框）：上边框嵌「会话」标题与「＋ 新建 (n)」提示，
内部每行一个会话——当前会话 ● 高亮、已关闭带 ✕ 标记；会话超出视口时，滚动过程中
会出现**暂态滚动条**（停止约 1 秒后隐藏，静止时不留灰条），边栏与聊天内容之间留有
一列空隙；transcript 自身带常驻滚动条。

输入框带**两类内建补全**（VSCode 式浮层下拉：悬于输入框上方、不推挤 transcript；
`↑↓` 选择、`Tab`/`Enter` 应用、`Esc` 关闭）：

- **`/` 命令补全**：空编辑器键入 `/` 列出全部命令，继续输入模糊过滤；`/context`、
  `/switch` 还提供参数候选（如 `/context re` → `reset`、`/switch d` → `draft`）。
- **`@` 文件引用**：任意位置在行首或空白后键入 `@` 列出文件/目录（`@src/` 继续下钻），
  选中的补全值直接插入消息文本供模型引用。以 [`fd`](https://github.com/sharkdp/fd)
  为唯一引擎（模糊子串检索 + 打分排序：尊重 .gitignore、嵌套 git 仓库按子树判定、
  单次扫描上限 100、取前 20）；未安装 `fd` 时 `@` 不出候选，仅提示一次。
  `@` 要求词元边界，避免把邮箱 `user@host` 之类误当文件引用。

交互**全部通过键盘完成**（不依赖鼠标按键）：
- **`Alt+S` 聚焦会话边栏**：`↑↓` 移动选中行（环绕、自动滚动跟随），`Enter` 打开会话，
  `n` 新建草稿（等同 `/new`），`d` 弹出删除确认菜单（Enter 确认、Esc 取消，等同
  `/delete`；删除当前会话会先释放其执行上下文并转入草稿态），`Tab` 切到转录浏览，`Esc`
  返回输入框；直接键入任意文字也会立刻回到输入框。会话按**作用域分组**显示
  （⎇ 主工作区在最上，其余为各 worktree）。
- **`Alt+T` 进入转录浏览**：`↑↓` 逐行滚动，`f` 展开/收起全部思考块，`Tab` 切回边栏，
  `Esc` 返回输入框；`PageUp`/`PageDown`/`Home`/`End` 翻页跳转（pi-tui 内置）。
- **`Alt+↑`/`Alt+↓` 循环切换 agent 标签**（右上角标签栏，标签属于当前会话）。

### 并行会话与 worktree 作用域

会话的**执行与界面焦点是解耦的**：会话 A 正在流式输出时可以随时切到会话 B 继续工作，
A 在后台照常跑，输出写进自己的缓冲，切回时**完整补放**（不丢字、不重放 JSONL）。
侧栏行内 `⏳` 标记输出中的会话、`•` 标记有未读输出的会话；双击 Esc 只中止**聚焦会话**。
每个会话有独立的 supervisor 上下文和专属子 agent 实例（互不串扰），任务派发只在
真正开始时懒创建。

worktree 是**包含会话的作用域容器**：先切入 worktree，再在其中创建会话；会话创建时
归属当前作用域并终身不变（没有改绑/迁移的概念）。

- `/worktree [名称]`：切入指定 worktree（不存在则基于 HEAD 创建同名分支）；
  名称省略时随机新建；`/worktree .` 切回主工作区。切入后自动恢复该作用域最近的
  活跃会话，没有则进入草稿；编辑器下方的状态栏显示当前作用域（`⎇ 名称`）。
- `/worktrees`：列出全部作用域；`/sessions` 按作用域分组列出。
- worktree 目录固定在 `<仓库根>/.pi-swarm/worktrees/<名称>`（自动写入 .gitignore），
  目录即该作用域内所有会话与子 agent 的工作目录；脏变更属于 worktree 本身，
  「换 worktree 继续改」请先在旧作用域提交。详见
  [docs/parallel-worktree-design.md](docs/parallel-worktree-design.md)。
...
首次发送任务时才真正创建会话（有草稿名用草稿名，否则用首条消息摘要命名）；
重复按 `n` 只是重新打开同一张草稿。首个任务、`/new` `/switch` `/close`
及边栏操作之后，列表统一刷新。右上角另有 agent 标签栏（supervisor 即根节点）。
`/status` 查看当前配置与已加载 agent；输入 `/exit` 或 `/quit` 才会结束进程。
