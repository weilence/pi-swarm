...
垂直排列，右侧为 transcript 与底部输入框）：上边框嵌「会话」标题，内部「＋ 新建」（等同
`/new`）加每行一个会话——当前会话 ● 高亮、已关闭带 ✕ 标记；**在会话行上点击右键**会
弹出上下文菜单（定位在点击处，含「删除会话」，等同 `/delete`；删除当前会话会先解绑
agent 并转入草稿态）；会话超出视口时，滚动过程中
会出现**暂态滚动条**（停止约 1 秒后隐藏，静止时不留灰条），边栏与聊天内容之间留有
一列空隙；transcript 自身带常驻滚动条。会话是**草稿式懒创建**：启动不建会话，
边栏显示「暂无会话」占位提示；「＋ 新建」（等同 `/new`）只打开一张**未保存草稿**
（清空 transcript、边栏顶部出现「✎ 草稿」行，可先 /model /thinking 配置），
首次发送任务时才真正创建会话（有草稿名用草稿名，否则用首条消息摘要命名）；
重复点击「新建」只是重新打开同一张草稿。首个任务、`/new` `/switch` `/close`
及点击操作之后，列表统一刷新。右上角另有 agent 标签栏（supervisor 即根节点）。
