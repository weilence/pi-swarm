...
console.log(
  interactive
    ? "输入任务，/provider /model /thinking 打开选择弹窗，/status 查看状态，或 /exit 退出。\n"
    : "输入任务，/provider <id> [接口类型]，/model [模型]，/thinking level，/status，或输入 /exit 退出。\n"
);
...
if (interactive) {
  ...
  const instance = render(
    createElement(InkApp, { store, worker, catalog: modelsCatalog, supervisor, module, onExit: settleExit }),
    { exitOnCtrlC: false }
  );
  await exited;
  instance.unmount();
